...
@[target, simp]
theorem id_comp (F : L →ᴸ L') : LHom.id L' ∘ᴸ F = F := by sorry

@[target, simp]
theorem comp_id (F : L →ᴸ L') : F ∘ᴸ LHom.id L = F := by sorry

@[target]
theorem comp_assoc {L3 : Language} (F : L'' →ᴸ L3) (G : L' →ᴸ L'') (H : L →ᴸ L') :
    F ∘ᴸ G ∘ᴸ H = F ∘ᴸ (G ∘ᴸ H) := by sorry
...
@[target]
theorem sumElim_inl_inr : LHom.sumInl.sumElim LHom.sumInr = LHom.id (L.sum L') := by sorry

@[target]
theorem comp_sumElim {L3 : Language} (θ : L' →ᴸ L3) :
    θ ∘ᴸ ϕ.sumElim ψ = (θ ∘ᴸ ϕ).sumElim (θ ∘ᴸ ψ) := by sorry
...
@[target, simp]
theorem sumMap_comp_inr : ϕ.sumMap ψ ∘ᴸ LHom.sumInr = LHom.sumInr ∘ᴸ ψ := by sorry
...
theorem map_onFunction {M : Type*} [L.Structure M] [L'.Structure M] [ϕ.IsExpansionOn M] {n}
    (f : L.Functions n) (x : Fin n → M) : funMap (ϕ.onFunction f) x = funMap f x := by sorry

@[target, simp]
theorem map_onRelation {M : Type*} [L.Structure M] [L'.Structure M] [ϕ.IsExpansionOn M] {n}
    (R : L.Relations n) (x : Fin n → M) : RelMap (ϕ.onRelation R) x = RelMap R x := by sorry
...
theorem funMap_sumInl [(L.sum L').Structure M] [(LHom.sumInl : L →ᴸ L.sum L').IsExpansionOn M] {n}
    {f : L.Functions n} {x : Fin n → M} : @funMap (L.sum L') M _ n (Sum.inl f) x = funMap f x := by sorry

@[target, simp]
theorem funMap_sumInr [(L'.sum L).Structure M] [(LHom.sumInr : L →ᴸ L'.sum L).IsExpansionOn M] {n}
    {f : L.Functions n} {x : Fin n → M} : @funMap (L'.sum L) M _ n (Sum.inr f) x = funMap f x := by sorry

@[target]
theorem sumInl_injective : (LHom.sumInl : L →ᴸ L.sum L').Injective := by sorry
...
theorem Injective.isExpansionOn_default {ϕ : L →ᴸ L'}
    ...
    @IsExpansionOn L L' ϕ M _ (ϕ.defaultExpansion M) := by sorry
...
@[target]
theorem constantsOn_constants : (constantsOn α).Constants = α := by sorry
...
@[target]
theorem card_constantsOn : (constantsOn α).card = #α := by sorry
...
theorem constantsOnMap_isExpansionOn {f : α → β} {fα : α → M} {fβ : β → M} (h : fβ ∘ f = fα) :
    @LHom.IsExpansionOn _ _ (LHom.constantsOnMap f) M (constantsOn.structure fα)
      (constantsOn.structure fβ) := by sorry
...
theorem card_withConstants :
    L[[α]].card = Cardinal.lift.{w'} L.card + Cardinal.lift.{max u v} #α := by sorry
...
@[target]
theorem lhomWithConstants_injective : (L.lhomWithConstants α).Injective := by sorry
...
theorem LHom.map_constants_comp_sumInl {f : α → β} :
    (L.lhomWithConstantsMap f).comp LHom.sumInl = L.lhomWithConstants β := by sorry
...
theorem withConstants_funMap_sumInr {a : α} {x : Fin 0 → M} :
    @funMap (L[[α]]) M _ 0 (Sum.inr a : L[[α]].Functions 0) x = L.con a := by sorry
...
@[target, simp]
theorem coe_con {a : A} : (L.con a : M) = a := by sorry
