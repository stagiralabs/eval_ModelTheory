...
@[target]
theorem subtype_injective (S : L.ElementarySubstructure M): Function.Injective (subtype S) := by sorry

@[target, simp]
theorem coe_subtype (S : L.ElementarySubstructure M) : ⇑S.subtype = Subtype.val := by sorry
...
@[target, simp]
theorem mem_top (x : M) : x ∈ (⊤ : L.ElementarySubstructure M) := by sorry

@[target, simp]
theorem coe_top : ((⊤ : L.ElementarySubstructure M) : Set M) = Set.univ := by sorry

@[target, simp]
theorem realize_sentence (S : L.ElementarySubstructure M) (φ : L.Sentence) : S ⊨ φ ↔ M ⊨ φ := by sorry

@[target, simp]
theorem theory_model_iff (S : L.ElementarySubstructure M) (T : L.Theory) : S ⊨ T ↔ M ⊨ T := by sorry
...
@[target]
theorem elementarilyEquivalent (S : L.ElementarySubstructure M) : S ≅[L] M := by sorry
