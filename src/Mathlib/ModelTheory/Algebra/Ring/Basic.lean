...
@[target]
theorem zero_def (α : Type*) : (0 : Language.ring.Term α) = Constants.term zeroFunc := by sorry
...
theorem add_def (α : Type*) (t₁ t₂ : Language.ring.Term α) :
    t₁ + t₂ = addFunc.apply₂ t₁ t₂ := by sorry
...
@[target, simp]
theorem card_ring : card Language.ring = 5 := by sorry
...
theorem realize_mul (x y : ring.Term α) (v : α → R) :
    Term.realize v (x * y) = Term.realize v x * Term.realize v y := by sorry
...
@[target, simp]
theorem realize_zero (v : α → R) : Term.realize v (0 : ring.Term α) = 0 := by sorry

@[target, simp]
theorem realize_one (v : α → R) : Term.realize v (1 : ring.Term α) = 1 := by sorry
