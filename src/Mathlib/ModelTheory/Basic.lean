...
theorem card_eq_card_functions_add_card_relations :
    L.card =
      (Cardinal.sum fun l => Cardinal.lift.{v} #(L.Functions l)) +
        Cardinal.sum fun l => Cardinal.lift.{u} #(L.Relations l) := by sorry
...
@[target, simp]
theorem card_empty : Language.empty.card = 0 := by sorry
...
@[target]
theorem funMap_eq_coe_constants {c : L.Constants} {x : Fin 0 → M} : funMap c x = c := by sorry
...
theorem HomClass.strongHomClassOfIsAlgebraic [L.IsAlgebraic] {F M N} [L.Structure M] [L.Structure N]
    [FunLike F M N] [HomClass L F M N] : StrongHomClass L F M N := by sorry

@[target]
theorem HomClass.map_constants {F M N} [L.Structure M] [L.Structure N] [FunLike F M N]
    [HomClass L F M N] (φ : F) (c : L.Constants) : φ c = c := by sorry
...
@[target, ext]
theorem ext ⦃f g : M →[L] N⦄ (h : ∀ x, f x = g x) : f = g := by sorry
...
@[target, simp]
theorem map_constants (φ : M →[L] N) (c : L.Constants) : φ c = c := by sorry

@[target, simp]
theorem map_rel (φ : M →[L] N) {n : ℕ} (r : L.Relations n) (x : Fin n → M) :
    RelMap r x → RelMap r (φ ∘ x) := by sorry
...
@[target, simp]
theorem comp_apply (g : N →[L] P) (f : M →[L] N) (x : M) : g.comp f x = g (f x) := by sorry
...
@[target, simp]
theorem comp_id (f : M →[L] N) : f.comp (id L M) = f := by sorry

@[target, simp]
theorem id_comp (f : M →[L] N) : (id L N).comp f = f := by sorry
...
@[target, simp]
theorem coe_toHom {f : M ↪[L] N} : (f.toHom : M → N) = f := by sorry

@[target]
theorem coe_injective : @Function.Injective (M ↪[L] N) (M → N) (↑) := by sorry

@[target, ext]
theorem ext ⦃f g : M ↪[L] N⦄ (h : ∀ x, f x = g x) : f = g := by sorry

@[target]
theorem toHom_injective : @Function.Injective (M ↪[L] N) (M →[L] N) (·.toHom) := by sorry

@[target, simp]
theorem toHom_inj {f g : M ↪[L] N} : f.toHom = g.toHom ↔ f = g := by sorry
...
theorem comp_injective (h : N ↪[L] P) :
    Function.Injective (h.comp : (M ↪[L] N) →  (M ↪[L] P)) := by sorry

@[target, simp]
theorem comp_inj (h : N ↪[L] P) (f g : M ↪[L] N) : h.comp f = h.comp g ↔ f = g := by sorry

@[target]
theorem toHom_comp_injective (h : N ↪[L] P) :
    Function.Injective (h.toHom.comp : (M →[L] N) →  (M →[L] P)) := by sorry

@[target, simp]
theorem toHom_comp_inj (h : N ↪[L] P) (f g : M →[L] N) : h.toHom.comp f = h.toHom.comp g ↔ f = g := by sorry

@[target, simp]
theorem comp_toHom (hnp : N ↪[L] P) (hmn : M ↪[L] N) :
    (hnp.comp hmn).toHom = hnp.toHom.comp hmn.toHom := by sorry

@[target, simp]
theorem comp_refl (f : M ↪[L] N) : f.comp (refl L M) = f := by sorry

@[target, simp]
theorem refl_comp (f : M ↪[L] N) : (refl L N).comp f = f := by sorry

@[target, simp]
theorem refl_toHom : (refl L M).toHom = Hom.id L M := by sorry
...
theorem symm_symm (f : M ≃[L] N) :
    f.symm.symm = f := by sorry

@[target]
theorem symm_bijective : Function.Bijective (symm : (M ≃[L] N) → _) := by sorry

@[target, simp]
theorem apply_symm_apply (f : M ≃[L] N) (a : N) : f (f.symm a) = a := by sorry

@[target, simp]
theorem symm_apply_apply (f : M ≃[L] N) (a : M) : f.symm (f a) = a := by sorry
...
@[target, simp]
theorem toEmbedding_toHom (f : M ≃[L] N) : f.toEmbedding.toHom = f.toHom := by sorry

@[target, simp]
theorem coe_toHom {f : M ≃[L] N} : (f.toHom : M → N) = (f : M → N) := by sorry

@[target, simp]
theorem coe_toEmbedding (f : M ≃[L] N) : (f.toEmbedding : M → N) = (f : M → N) := by sorry
...
@[target, simp]
theorem refl_apply (x : M) : refl L M x = x := by sorry
...
@[target, simp]
theorem refl_comp (g : M ≃[L] N) : (refl L N).comp g = g := by sorry

@[target, simp]
theorem refl_toEmbedding : (refl L M).toEmbedding = Embedding.refl L M := by sorry
...
@[target, simp]
theorem symm_comp_self (f : M ≃[L] N) : f.symm.comp f = refl L M := by sorry

@[target, simp]
theorem symm_comp_self_toEmbedding (f : M ≃[L] N) :
    f.symm.toEmbedding.comp f.toEmbedding = Embedding.refl L M := by sorry

@[target, simp]
theorem self_comp_symm_toEmbedding (f : M ≃[L] N) :
    f.toEmbedding.comp f.symm.toEmbedding = Embedding.refl L N := by sorry

@[target, simp]
theorem symm_comp_self_toHom (f : M ≃[L] N) :
    f.symm.toHom.comp f.toHom = Hom.id L M := by sorry
...
theorem comp_right_injective (h : M ≃[L] N) :
    Function.Injective (fun f ↦ f.comp h : (N ≃[L] P) → (M ≃[L] P)) := by sorry

@[target, simp]
theorem comp_right_inj (h : M ≃[L] N) (f g : N ≃[L] P) : f.comp h = g.comp h ↔ f = g := by sorry
...
theorem funMap_sumInr {n : ℕ} (f : L₂.Functions n) :
    @funMap (L₁.sum L₂) S _ n (Sum.inr f) = funMap f := by sorry

@[target, simp]
theorem relMap_sumInl {n : ℕ} (R : L₁.Relations n) :
    @RelMap (L₁.sum L₂) S _ n (Sum.inl R) = RelMap R := by sorry

@[target, simp]
theorem relMap_sumInr {n : ℕ} (R : L₂.Relations n) :
    @RelMap (L₁.sum L₂) S _ n (Sum.inr R) = RelMap R := by sorry
...
theorem empty.nonempty_equiv_iff :
    Nonempty (M ≃[Language.empty] N) ↔ Cardinal.lift.{w'} #M = Cardinal.lift.{w} #N := by sorry
...
theorem toEquiv_inducedStructureEquiv (e : M ≃ N) :
    @Language.Equiv.toEquiv L M N _ (inducedStructure e) (inducedStructureEquiv e) = e := by sorry

@[target, simp]
theorem toFun_inducedStructureEquiv (e : M ≃ N) :
    DFunLike.coe (@inducedStructureEquiv L M N _ e) = e := by sorry

@[target, simp]
theorem toFun_inducedStructureEquiv_Symm (e : M ≃ N) :
    (by
    letI : L.Structure N := inducedStructure e
    exact DFunLike.coe (@inducedStructureEquiv L M N _ e).symm) = (e.symm : N → M) := by sorry
